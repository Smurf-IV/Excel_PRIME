...
namespace System.Collections.Generic
{
    ...
    {
        private static readonly Type? s_type = typeof(IDictionary).Assembly.GetType("System.Collections.HashHelpers");

        public static uint HashCollisionThreshold { get; } = (uint?)s_type?.GetField("HashCollisionThreshold", BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.Public)?.GetValue(null) ?? 100u;

        private static readonly Func<int, int>? s_getPrime = s_type?.GetMethod("GetPrime", BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.Public, null, new[] { typeof(int) }, null)?.CreateDelegate<Func<int, int>>();
        private static readonly Func<int, int>? s_expandPrime = s_type?.GetMethod("ExpandPrime", BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.Public, null, new[] { typeof(int) }, null)?.CreateDelegate<Func<int, int>>();
        private static readonly Func<uint, ulong>? s_getFastModMultiplier = s_type?.GetMethod("GetFastModMultiplier", BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.Public, null, new[] { typeof(uint) }, null)?.CreateDelegate<Func<uint, ulong>>();

        private delegate uint FastModDelegate(uint value, uint divisor, ulong multiplier);
        private static readonly FastModDelegate? s_fastMod = s_type?.GetMethod("FastMod", BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.Public, null, new[] { typeof(uint), typeof(uint), typeof(ulong) }, null)?.CreateDelegate<FastModDelegate>();

        public static int GetPrime(int min) => s_getPrime?.Invoke(min) ?? min;
        public static int ExpandPrime(int oldSize) => s_expandPrime?.Invoke(oldSize) ?? (oldSize * 2);
        public static ulong GetFastModMultiplier(uint divisor) => s_getFastModMultiplier?.Invoke(divisor) ?? (ulong.MaxValue / divisor + 1);
        public static uint FastMod(uint value, uint divisor, ulong multiplier) => s_fastMod?.Invoke(value, divisor, multiplier) ?? (uint)(((((multiplier * value) >> 32) + 1) * divisor) >> 32);
    }

    internal static class ThrowHelper
    {
        private static readonly Type? s_type = typeof(IDictionary).Assembly.GetType("System.Collections.ThrowHelper");

        [DoesNotReturn]
        public static void ThrowDuplicateKey(object key) => Invoke(nameof(ThrowDuplicateKey), key);
        [DoesNotReturn]
        public static void ThrowIndexArgumentOutOfRange() => Invoke(nameof(ThrowIndexArgumentOutOfRange));
        [DoesNotReturn]
        public static void ThrowConcurrentOperation() => Invoke(nameof(ThrowConcurrentOperation));
        [DoesNotReturn]
        public static void ThrowVersionCheckFailed() => Invoke(nameof(ThrowVersionCheckFailed));

        [DoesNotReturn]
        private static void Invoke(string methodName, params object?[] args)
        {
            var method = s_type?.GetMethod(methodName, BindingFlags.NonPublic | BindingFlags.Static);
            if (method != null)
            {
                try
                {
            ...
            // Fallback
            throw methodName switch
            {
                nameof(ThrowDuplicateKey) => new ArgumentException($"An item with the same key has already been added. Key: {args[0]}"),
                nameof(ThrowIndexArgumentOutOfRange) => new ArgumentOutOfRangeException("index"),
                nameof(ThrowConcurrentOperation) => new InvalidOperationException("Collection was modified; enumeration operation may not execute."),
                nameof(ThrowVersionCheckFailed) => new InvalidOperationException("Collection was modified; enumeration operation may not execute."),
                _ => new InvalidOperationException(methodName)
            };
        }
    }
    ...
    internal static class SR
    {
        ...
        public const string Arg_WrongType = "The value '{0}' is not of type '{1}' and cannot be used in this collection.";
        ...
        public const string Arg_HTCapacityOverflow = "Hashtable's capacity overflowed because it exceeded max capacity.";
    ...
    {
        public static IEnumerator<T> GetEmptyEnumerator<T>() => Enumerable.Empty<T>().GetEnumerator();
