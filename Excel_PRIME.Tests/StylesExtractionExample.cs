        ...
        foreach (IRow? row in sheet.GetRowData(ct: ct))
        {
            ...
            foreach (ICell? cell in cells)
            {
                ...
                try
                {
                    ...
                    {
                        >= 1000 => numValue.ToString("N2"), // 1,234.56
                        < 1 and > 0 => numValue.ToString("P"), // 50.00%
...
public static class CellStyleExtensions
{
    ...
    /// </summary>
    public static bool HasNumericFormat(this CellStyle style) => !string.IsNullOrEmpty(style?.Formatting);
    ...
    {
        if (string.IsNullOrEmpty(style?.Formatting))
        {
            return false;
        }

        ReadOnlySpan<char> span = style.Formatting.AsSpan();
    ...
    /// </summary>
    public static bool HasPercentageFormat(this CellStyle style) => style?.Formatting?.Contains('%') == true;
    ...
    public static int GetDecimalPlaces(this CellStyle style)
    {
        ...
        foreach (char c in style.Formatting)
        {
            if (c == '.')
            {
                foundDecimal = true;
